  ...
  context 'basic CRUD' do
    context 'creating' do
      ...
      it 'can be created with a hash of attributes' do
        ...
        expect(Movie.find_by(title: "The Color Purple")).to eq(movie)
      end
    ...
    context 'reading' do
      ...
      it 'can retrieve from the database using different attributes' do
        ...
        expect(Movie.find_by_title("Title")).to eq(movie)
      end

      it 'can use a where clause and be sorted' do
        expect(Movie.where(title: ["Movie_4", "Movie_3"]).order(title: :desc).map{|m| m.title}).to eq(["Movie_4", "Movie_3"])
      end
    end
